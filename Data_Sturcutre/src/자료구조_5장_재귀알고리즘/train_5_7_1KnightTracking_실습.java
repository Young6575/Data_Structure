...
public class train_5_7_1KnightTracking_실습 {
    ...
    private static boolean solveKnightTracking(int startX, int startY) {
        ...
        while (!stack.isEmpty()) {
           ...
           boolean moved = false;
           // 8가지 방향을 모두 방문했는지 확인.
           for (int i=currentPoint.moveToward;i<8;i++) {
        	   int nextX = currentPoint.x + moves[i].a;
        	   int nextY = currentPoint.y + moves[i].b;
        	   
        	   	if(isSafe(nextX, nextY)) {
	        		   stack.push(new Point(nextX, nextY, 0));
	        		   board[nextX][nextY] = moveCount++;
	        		   System.out.println("이동 -> (" + nextX + ", " + nextY + ") = " + moveCount);
	        		   moved = true;
	        		   currentPoint.moveToward = i + 1;
	        		   break;
        	   }
           	}
		   	   	if (moved == false) {
